...
$(document).ready(function() {
    ...
    $(document).on('click', '.quitar', function() {
        ...
        if ($seleccionados.length != 0) {
            var nroCandidatosSeleccionados = $.map($seleccionados, function(row) {
                return row.nroCandidato;
            });

            $('#table-pagination2').bootstrapTable('remove', {
                field: 'nroCandidato',
                values: nroCandidatosSeleccionados
            });
            ...
            $candidatosAevaluar = $('#table-pagination2').bootstrapTable('getData');

            if ($candidatosAevaluar.length != 0) {
                var nroCandidatosAevaluar = $.map($candidatosAevaluar, function(row) {
                return row.nroCandidato;
                });

                $('#table-pagination').bootstrapTable('remove', {
                field: 'nroCandidato',
                values: nroCandidatosAevaluar
                });    
                ...
                for (var i=0, total = nroCandidatosAevaluar.length; i < total; i++) {
                    ...
                    newFormLi = $('<div class="row renglon"></div>').append($('<div class="col-md-2"></div>').append(
                           ...
                           +']" value="'+ nroCandidatosAevaluar[i] +'" type="hidden">'));
        ...
        if ($seleccionados.length == 0)
            ...
            success: function(data){
                ...
                if(data.length != 0){
                    ...
                    '<span class="text-danger"><ul><li>'+data[$i].nroCandidato+'  -  '+data[$i].apellido+', '+data[$i].nombre+'</li></ul></span>'));
                    }
    ...
    $(document).on('click', '.agregar', function() {
        ...
        if ($seleccionados.length != 0) { 
            $nroCandidatos = $.map($seleccionados, function(row) {
                return row.nroCandidato;
            });
            ...
            $nroCandidatosAevaluar = $.map($candidatosAevaluar, function(row) {
                return row.nroCandidato;
            });
            ...
            $collectionHolder = $('div.candidatos');
               ...
               for (var i=0, total = $nroCandidatosAevaluar.length; i < total; i++) {
                   ...
                   newFormLi = $('<div class="row renglon"></div>').append($('<div class="col-md-2"></div>').append(
                           '<input id="form_candidatos_'+i+'" class="form-control" name="form[candidatos]['+i+']" value="'+ $nroCandidatosAevaluar[i] 
                           +'"  type="hidden">'));
